...
transform = transforms.Compose([
    transforms.Resize((600, 600)),  # EfficientNet-B7에 적합한 해상도
    transforms.RandomHorizontalFlip(),  # 좌우 대칭
    transforms.RandomRotation(10),  # 10도 회전
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),  # 밝기, 대비, 색상 조정
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])
...
plt.plot(epochs, train_accuracies_dict[0], label=f'Train Acc B7', color='yellow', linestyle='-')
plt.plot(epochs, valid_accuracies_dict[0], label=f'Valid Acc B7', color='yellow', linestyle='--')
plt.title('Train and Valid Accuracy for B7')
plt.xlabel('Epochs')
plt.ylabel('Accuracy (%)')
plt.legend()
plt.grid(True)
current_time = get_current_time()
plt.savefig(os.path.join(save_path, f'{current_time}_b7_train_valid_accuracy.png'))
plt.close()

# Test Accuracy Bar Plot
plt.figure(figsize=(14, 7))
plt.bar(['B7'], [test_accuracy], color=['yellow'])
plt.title('Test Accuracy for B7')
plt.xlabel('Model')
plt.ylabel('Accuracy (%)')
current_time = get_current_time()
plt.savefig(os.path.join(save_path, f'{current_time}_b7_test_accuracy_bar_plot.png'))
plt.close()

# Best performance 출력
print("\nBest Performance Summary:")
print(f"EfficientNet-B7: Best Train Acc: {best_performance['best_train_acc']:.2f}%, Best Valid Acc: {best_performance['best_valid_acc']:.2f}%, Test Acc: {best_performance['test_acc']:.2f}%, Best Epoch: {best_performance['best_epoch']}")
